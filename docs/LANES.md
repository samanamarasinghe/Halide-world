...
## The data lane wrote across the site lane's claim, three times, on his instruction

`build_site.py` belongs to the site lane. It has been edited from the data lane three
times, each because he said to: once to render the artifact-edge repos, on 2026-08-19 via
`patch_person_edges.py` to point the person layer at `contributor_edges.json` and
`affiliation_edges.json`, and on 2026-08-21 via `patch_author_merges.py` to APPLY
`person_aliases.json` `author_merges` in `build_people` — `curate/person_aliases.py` only
validates the file, and nothing consumed it, so all 348 reviewed merges (incl. the five
Amarasinghe cards) had no effect on the site. The patch remaps each author id to its keep
id, names the node from the alias entry, and registers absorbed spellings in the by-name
join so anchor authorship still lands on the merged node. All are flagged rather than
silent.
